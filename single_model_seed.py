...
def process_data(filename, training_flag, features, impute, standardize, whiten, y_column, replace_val, replace_by):
    ...
    if replace_val is not None:
        ...
        print '   ==> Verify No. of ' + str(replace_val) + ': ' + str(np.sum(X==replace_val))
...
def train(X, y, alg, scaler, pca, features):
    ...
    if 'xgboost' in alg:
        ...
        return train_xgb(X, y, scaler, pca, seeder)
...
def train_xgb(X, y, scaler, pca, seeder):
    """
    ...
    param['objective'] = 'binary:logistic'     # no change from original setting
    ...
    t1 = time.time()
    print "~" * 80
    print '   ==> Used Random Seed (%i) - Model Trained in %.2f secs.' % (seeder, t1-t0)
...
def main():
    ...
    alg = 'xgboost8340'  # bayes, logistic, svm, boost, xgboostnnnn
    features = 10
    ...
    print "~" * 80
    print 'Process Complete. And the Crowd Goes Wild.'
